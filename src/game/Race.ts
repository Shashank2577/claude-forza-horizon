...
import { GRID_LAT, GRID_ROW_DS } from '../world/TrackLayout';
...
export class Race implements IRace {
  ...
  /**
   * How the session is being played. FreeRoam disables the lap director, the
   * finish and wrong-way entirely; Sprint runs one point-to-point "lap".
   * Circuit is the adopted kart-royale race, untouched. A private field with a
   * readonly accessor, so nothing outside can flip modes under our feet.
   */
  private _mode: RaceMode = RaceMode.Circuit;

  get mode(): RaceMode {
    return this._mode;
  }
  /**
   * Sprint finish station (normalised t). 0 means "wrap at the line" — i.e.
   * not a sprint — which is how `updateProgress` tells the two apart.
   */
  private sprintFinishT = 0;
  /** true once the player has crossed their sprint finish gate */
  private sprintDone = false;

  /** Checkpoint index of the sprint finish gate (0 in circuit mode). */
  private sprintGate(): number {
    if (this.sprintFinishT <= 0) return 0;
    return Math.floor(this.sprintFinishT * this.ctx.track.checkpointCount);
  }
  ...
  reset() {
    // A fresh session is always a circuit race again — roam/sprint are entered
    // explicitly through their own entry points, never inherited by a restart.
    this._mode = RaceMode.Circuit;
    this.sprintFinishT = 0;
  ...
  /**
   * Enter free roam. No countdown, no laps, no finish: the field is parked at
   * scenic spots along the circuit and the player just drives. The state is
   * `FreeRoam`, which every consumer of `IRace` treats as "live but untimed" —
   * the HUD hides the lap chip and the clock, the stunts layer arms, and the
   * AI karts cruise rather than race.
   */
  startRoam() {
    this._mode = RaceMode.FreeRoam;
    this.sprintFinishT = 0;
    this.sprintDone = false;
    this._state = RaceState.FreeRoam;
    this.raceTime = 0;
    this.resultsT = 0;
    this.wrongWay = false;
    this.finishedCount = 0;
  ...
  /**
   * Enter a sprint: the field forms up at `fromT` behind a standing start and
   * runs one point-to-point leg to `toT`. Deliberately reuses the countdown,
   * checkpoint and finish plumbing — `updateProgress` treats station `toT` as
   * the line whenever `sprintFinishT > 0`, so there is no second race
   * director to keep in sync.
   */
  startSprint(fromT = 0.02, toT = 0.52) {
    this._mode = RaceMode.Sprint;
    this.sprintFinishT = toT;
    this.sprintDone = false;
    this.start();
    this.formGridAt(fromT);
  }

  /**
   * Free-roam parking. Same placement maths as `formGrid` but at scenic
   * stations instead of the start boxes, spread around the island so the
   * opening shot reads as a world rather than a queue.
   */
  private scatterForRoam() {
    const track = this.ctx.track;
    const spots = [0.03, 0.17, 0.31, 0.45, 0.59, 0.73, 0.87, 0.96];
    for (let i = 0; i < this.karts.length; i++) {
      const k = this.karts[i];
      const frac = k.isPlayer ? spots[0] : spots[(i + 3) % spots.length];
      const s = track.sample(frac);
      // Rivals park just off the racing line, alternating sides, so a player
      // who spawns and holds throttle does not plough them on the way past.
      const lat = k.isPlayer ? 0 : 3.4 * (i % 2 ? 1 : -1);
      const maxLat = Math.max(1.5, s.halfWidth - GRID_MARGIN);
      _v.copy(s.pos)
        .addScaledVector(s.binormal, clamp(lat, -maxLat, maxLat))
        .addScaledVector(s.normal, GRID_LIFT);
      k.placeAt(_v, Math.atan2(s.tangent.x, s.tangent.z), frac);
      k.lap = 0;
      k.finished = false;
      k.place = i + 1;
      k.raceDistance = 0;
  ...
  /**
   * Sprint formation at station `fromT`. This is `formGrid` transplanted to an
   * arbitrary station: same staggered two-column maths, same slot-for-slot
   * player/pole trade, solved from a live sample instead of `track.startGrid`
   * (which is baked for the real line only).
   */
  private formGridAt(fromT: number) {
    const track = this.ctx.track;
    const N = track.checkpointCount;
    for (let i = 0; i < this.karts.length; i++) {
      const k = this.karts[i];
      const gi = this.slotFor(i);
      // Two columns, staggered back — identical geometry to the start grid.
      const s = track.sample(fromT - gi * GRID_ROW_DS / track.length);
      const lat = ((gi % 2) ? -1 : 1) * GRID_LAT;
      const maxLat = Math.max(1.5, s.halfWidth - GRID_MARGIN);
      _v.copy(s.pos)
        .addScaledVector(s.binormal, clamp(lat, -maxLat, maxLat))
        .addScaledVector(s.normal, GRID_LIFT);
      k.placeAt(_v, Math.atan2(s.tangent.x, s.tangent.z), fromT);
      k.lap = 0;
      k.finished = false;
      k.place = i + 1;
      k.raceDistance = 0;
      ...
      const p = this.prog[i];
      p.lapIndex = 0;   // already across the line for this leg's purposes
      p.cp = track.checkpointAt(fromT);
      // If the formation happens to sit exactly on the finish gate's own
      // checkpoint, back everyone up one so the crossing still validates.
      if (p.cp === this.sprintGate()) p.cp = (p.cp + N - 1) % N;
      p.lapStart = 0;
      p.best = Infinity;
      p.finishOrder = 0;
      p.finishTime = 0;
  ...
  update(ctx: Ctx, dt: number) {
    ...
    } else if (this.state === RaceState.FreeRoam) {
      // Roam is untimed by design — the HUD hides the clock and the lap chip,
      // so advancing `raceTime` would only feed a readout nobody sees.
    ...
    const live = this.state === RaceState.Racing || this.state === RaceState.FreeRoam;
    ...
    for (let i = 0; i < this.karts.length; i++) {
      ...
      if (p.respawnT > 0) {
        ...
        if (k.isPlayer && !this.autoDrive && !k.finished && this.state !== RaceState.Results) {
          ...
          brake = cmd.brake;
          drift = cmd.drift;
          // Roam rivals are traffic, not racers: same line-following brain,
          // dethrottled so they tour the island at a sightseeing pace instead
          // of circulating at qualifying speed.
          if (this.state === RaceState.FreeRoam) throttle *= 0.42;
          if (cmd.useItem && this.state !== RaceState.FreeRoam) ctx.items.use(k, cmd.itemBackwards);
        }
  ...
  private updateProgress() {
    ...
    const N = track.checkpointCount;
    const cpLen = L / N;
    // In sprint mode the finish GATE is the line: crossing it in order
    // completes the leg, and there is no wrap-around lap counting at all.
    const gate = this.sprintGate();
    const sprinting = gate > 0 && this.mode === RaceMode.Sprint;
    ...
    for (let i = 0; i < this.karts.length; i++) {
      ...
      if (fwd === 1) {
        p.cp = c;
        if (sprinting) {
          // The leg ends at the gate, not back at the start line.
          if (c === gate && !k.finished) this.crossSprint(k, p);
        } else if (this.mode !== RaceMode.FreeRoam && c === 0) {
          this.crossLine(k, p);
        }
      } else if (fwd === N - 1) {
        ...
        // comes round again.
        p.cp = c;
        if (!sprinting && c === N - 1) p.lapIndex = Math.max(-1, p.lapIndex - 1);
      }
  ...
  /**
   * A sprint leg completes at the finish GATE rather than back at the start
   * line. Same bookkeeping as a lap — time stamped, order published, results
   * screen handed to once the player is home — but `lapIndex` never increments
   * and nobody circulates again.
   */
  private crossSprint(k: Kart, p: Progress) {
    const ctx = this.ctx;
  ...
  private watchdogs(ctx: Ctx, dt: number) {
    for (let i = 0; i < this.karts.length; i++) {
      ...
      // Free roam is a world, not a corridor: driving the fields and the beach
      // is the point. Only water cranes you back (a beached kart in the sea
      // has no other way out); grass and sand are legitimate roam terrain.
      const bad = this.mode === RaceMode.FreeRoam
        ? k.surface === Surface.Water
        : k.surface === Surface.Water || k.surface === Surface.OffTrack;
  ...
  private updateWrongWay(ctx: Ctx, dt: number, live: boolean) {
    ...
    // Free roam has no "wrong way" — there is no way to be wrong in a world.
    const backwards = live && this.mode !== RaceMode.FreeRoam &&
      along < -0.28 && Math.abs(this.player.forwardSpeed) > 3.5;
