 ...
 * ============================================================================
 *  Open-wheel speedster proportions on a GT silhouette: 0.36 m wheels under
 *  flared arches, a long low nose, a raked windscreen, twin roll-over hoops
 *  and a ducktail. Nothing here is a box: every part is lofted through chamfered
...
function buildChassis(): ChassisGeo {
  ...
  // ROADSTER CONVERSION. The tub walls came down by ~35%: a tall open tub is
  // the single strongest "kart" cue in the silhouette. A low doorline with a
  // shoulder-high sill reads as a speedster body, and it lets the windscreen —
  // not the bodywork — own the upper half of the profile.
  for (const s of [-1, 1]) {
    P.addLoft(
      [
        { z: -0.78, hw: 0.072, hh: 0.078, y: 0.31, r: 0.045 },
        { z: -0.52, hw: 0.078, hh: 0.118, y: 0.345, r: 0.05 },
        { z: -0.12, hw: 0.080, hh: 0.125, y: 0.355, r: 0.05 },
        { z: 0.24, hw: 0.076, hh: 0.112, y: 0.345, r: 0.05 },
        { z: 0.56, hw: 0.070, hh: 0.088, y: 0.325, r: 0.045 },
      ],
      Role.Base,
      mat(s * 0.437, 0, 0),
    ...
    P.addLoft(
      [
        { z: -0.70, hw: 0.055, hh: 0.014, y: 0.44, r: 0.011 },
        { z: -0.12, hw: 0.060, hh: 0.016, y: 0.465, r: 0.012 },
        { z: 0.50, hw: 0.052, hh: 0.014, y: 0.415, r: 0.011 },
      ],
      Role.Trim,
      mat(s * 0.437, 0, 0),
  ...
  // --- nose: LONG, low, and sweeping — the hood of a front-engined GT -------
  // Stretched forward (z 1.06 vs the kart's 1.00) and dropped: a long bonnet
  // falling away to a shallow grille is what separates "sports car" from
  // "kart with a pointy front". The crown height comes down ~25% so the
  // windscreen, not the nose, sets the tallest line ahead of the driver.
  P.addLoft(
    [
      { z: 0.46, hw: 0.46, hh: 0.115, y: 0.335, r: 0.085, taper: -0.10 },
      { z: 0.70, hw: 0.44, hh: 0.105, y: 0.320, r: 0.08, taper: -0.13 },
      { z: 0.90, hw: 0.385, hh: 0.092, y: 0.300, r: 0.075, taper: -0.18 },
      { z: 1.06, hw: 0.29, hh: 0.072, y: 0.278, r: 0.06, taper: -0.22 },
    ],
    Role.Base,
    undefined,
  ...
  // --- side pods: sleeker flank pontoons ----------------------------------
  for (const s of [-1, 1]) {
    ...
    P.addLoft(
      [
        { z: -0.42, hw: 0.128, hh: 0.105, y: 0.300, r: 0.05, taper: -0.10 },
        { z: -0.10, hw: 0.145, hh: 0.120, y: 0.300, r: 0.05, taper: -0.10 },
        { z: 0.18, hw: 0.140, hh: 0.112, y: 0.295, r: 0.05, taper: -0.10 },
        { z: 0.40, hw: 0.118, hh: 0.088, y: 0.290, r: 0.048, taper: -0.12 },
      ],
      Role.Base,
      mat(s * 0.635, 0, 0),
      { corner: 2, capStart: 0.05, capEnd: 0.05 },
    );
    // pod intake mouth, recessed and dark so the flank reads as sculpted
    M.addLoft(
      [
        { z: 0.02, hw: 0.052, hh: 0.058, y: 0.30, r: 0.028 },
        { z: 0.08, hw: 0.042, hh: 0.048, y: 0.30, r: 0.022 },
      ],
      Role.Plastic,
      mat(s * 0.635, 0, 0.30),
      { corner: 1, capStart: 0.02, capEnd: 0.02, capSeg: 1 },
  ...
  // --- rear deck: LOW haunches instead of an engine block -----------------
  // The kart's tall cooling block (hh 0.245 at y 0.50) is gone; a GT tail is a
  // broad, low deck that falls away to the ducktail. This is what kills the
  // "engine sticking out the back" kart read from the chase camera.
  P.addLoft(
    [
      { z: -0.98, hw: 0.32, hh: 0.115, y: 0.405, r: 0.07, taper: -0.08 },
      { z: -0.80, hw: 0.345, hh: 0.135, y: 0.415, r: 0.075, taper: -0.08 },
      { z: -0.60, hw: 0.335, hh: 0.128, y: 0.415, r: 0.072, taper: -0.10 },
    ],
    Role.Base,
    undefined,
  ...
  // Lower, shallower grille-height bumper for the GT nose — the kart's tall
  // chest-high bar read as a cow-catcher at closeup framing.
  bumper(0.085, 0.052, 0.245, Role.Base, P, 0, 3);
  ...
  bumper(0.030, 0.017, 0.242, Role.Steel, C, -0.078, 3);
  ...
  // --- roll hoops + ducktail ----------------------------------------------
  // The kart's tall light-bar hoop (peak y 1.31) is replaced by TWO LOW
  // speedster fairings hoops behind the seats, peak y 0.90 — just above the
  // driver's helmet, exactly where a roadster's roll-over hoops sit. The tall
  // rear wing is replaced by a ducktail lip on the deck's trailing edge.
  for (const s of [-1, 1]) {
    C.addTube(
      smoothPath([
        new THREE.Vector3(s * 0.30, 0.44, -0.34),
        new THREE.Vector3(s * 0.33, 0.68, -0.38),
        new THREE.Vector3(s * 0.28, 0.86, -0.36),
        new THREE.Vector3(s * 0.17, 0.935, -0.345),
        new THREE.Vector3(s * 0.055, 0.95, -0.34),
      ], 14),
      0.030, 11, Role.Steel,
    );
    // headrest fairing sweeping back from each hoop into the deck
    P.addLoft(
      [
        { z: 0.0, hw: 0.075, hh: 0.048, r: 0.035 },
        { z: 0.16, hw: 0.088, hh: 0.038, r: 0.03 },
      ],
      Role.Base,
      mat(s * 0.185, 0.80, -0.52, -Math.PI / 2 + 0.22),
      { corner: 2, capStart: 0.04, capEnd: 0.06 },
    );
  }
  // slim cross brace between the hoops — structural read without the bar
  C.addTube(
    [new THREE.Vector3(-0.30, 0.88, -0.365), new THREE.Vector3(0.30, 0.88, -0.365)],
    0.018, 7, Role.Steel,
  );

  // --- windscreen: raked glass canopy --------------------------------------
  // A single raked screen from the scuttle to above the driver. Chrome at this
  // size reads as tinted glass in env reflections; its rake (~41 deg) continues
  // the hood line instead of interrupting it, which is what keeps the profile
  // reading as one shape. Thin section so the edge catches a highlight line.
  C.addLoft(
    [
      { z: 0.0, hw: 0.425, hh: 0.020, y: 0, r: 0.012 },
      { z: 0.115, hw: 0.398, hh: 0.018, y: 0, r: 0.011 },
      { z: 0.235, hw: 0.360, hh: 0.018, y: 0, r: 0.011 },
    ],
    Role.Steel,
    mat(0, 0.462, 0.42, -Math.PI / 2 + 0.62),
    { corner: 1, capStart: 0.018, capEnd: 0.02, capSeg: 1 },
  );
  // screen surround bead in trim colour, hugging the glass edge
  P.addLoft(
    [
      { z: 0.0, hw: 0.432, hh: 0.010, y: 0, r: 0.006 },
      { z: 0.235, hw: 0.366, hh: 0.010, y: 0, r: 0.006 },
    ],
    Role.Trim,
    mat(0, 0.452, 0.43, -Math.PI / 2 + 0.62),
    { corner: 1, capStart: 0.01, capEnd: 0.01, capSeg: 1 },
  );

  // --- ducktail spoiler -----------------------------------------------------
  // Kicks up off the deck's trailing edge — the quietest possible way to say
  // "rear aero" without a kart wing on stilts.
  P.addLoft(
    [
      { z: -0.44, hw: 0.115, hh: 0.016, y: 0.555, r: 0.012 },
      { z: 0, hw: 0.125, hh: 0.019, y: 0.57, r: 0.014 },
      { z: 0.44, hw: 0.115, hh: 0.016, y: 0.555, r: 0.012 },
    ],
    Role.Trim,
    mat(0, 0, -1.00, 0, Math.PI / 2, 0),
    { corner: 2, capStart: 0.02, capEnd: 0.02, capSeg: 1 },
  );

  // --- exhaust: twin tailpipes under the tail ------------------------------
  // Short horizontal pipes exiting the underside of the rear deck, outboard
  // pair. The kart's tall raked stacks are gone; a GT exhaust exits low and
  // dark, and the boost flame anchors in buildKart follow this position.
  for (const s of [-1, 1]) {
    C.addTube(
      [
        new THREE.Vector3(s * 0.20, 0.30, -0.92),
        new THREE.Vector3(s * 0.24, 0.295, -1.06),
      ],
      (t) => 0.042 - t * 0.004,
      9, Role.Steel,
    );
  ...
  // --- seat, dash, column --------------------------------------------------
  // ROADSTER CONVERSION: the tall raked backrest and its bolsters are GONE —
  // they stood proud of the lowered tub like a green slab. The driver sits in
  // an open cockpit; the hoops and fairings behind him are the backrest now.
  M.addLoft(
    [
      { z: -0.40, hw: 0.215, hh: 0.038, y: 0.315, r: 0.026 },
      { z: -0.10, hw: 0.225, hh: 0.042, y: 0.308, r: 0.028 },
      { z: 0.06, hw: 0.205, hh: 0.038, y: 0.308, r: 0.026 },
    ],
    Role.Plastic,
    undefined,
    { corner: 1, capStart: 0.03, capEnd: 0.03, capSeg: 1 },
  );
  // low seat squab only — no backrest
  // headrest pad in the livery trim — REMOVED in the roadster conversion; the
  // twin fairing hoops behind the seats own that silhouette now.

  // dash + column shroud + pedals — dropped with the tub
  M.addLoft(
    [
      { z: -0.24, hw: 0.06, hh: 0.065, y: 0.42, r: 0.03 },
      { z: 0.24, hw: 0.06, hh: 0.065, y: 0.42, r: 0.03 },
    ],
    Role.Plastic,
  ...
  C.addTube(
    [new THREE.Vector3(0, 0.45, 0.44), new THREE.Vector3(0, 0.66, 0.30)],
    0.024, 8, Role.Steel,
  );
  ...
  // roll hoop.
  C.addGeometry(new THREE.TorusGeometry(0.042, 0.011, 3, 7), Role.Steel, mat(0.13, 0.50, 0.435, 0.35), 1.5);
  ...
  for (const s of [-1, 1]) {
    C.addTube(
      [new THREE.Vector3(s * 0.42, 0.52, 0.32), new THREE.Vector3(s * 0.53, 0.64, 0.30)],
      0.013, 5, Role.Steel,
    );
    P.addLoft(
      [
      ...
      Role.Trim,
      mat(s * 0.565, 0.665, 0.295, 0, s * 0.35, 0),
      { corner: 2, capStart: 0.018, capEnd: 0.018, capSeg: 1 },
    );
  }
  C.addLoft(
    [
      { z: 0.0, hw: 0.052, hh: 0.052, y: 0.525, r: 0.05 },
      { z: 0.022, hw: 0.048, hh: 0.048, y: 0.525, r: 0.046 },
    ],
    Role.Steel,
    mat(-0.24, 0, -0.62, Math.PI / 2),
  ...
  D.addPanel(pw, ph, 0.010, PANEL_UV.podL,
    mat(-0.784, 0.295, -0.01, 0, -Math.PI / 2, 0).multiply(mat(0, 0, 0, -0.10, 0, 0)), Role.Base, 5);
  ...
  D.addPanel(
    ...
    mat(0, 0.425, 0.78, 0.16, 0, 0).multiply(mat(0, 0, 0, 0, Math.PI, 0)).multiply(mat(0, 0, 0, -Math.PI / 2, 0, 0)),
    Role.Base, 6,
  );
  // tail number plate — the panel the player stares at all race
  const [tw, th] = PANEL_SIZE.tail;
  D.addPanel(tw, th, 0.010, PANEL_UV.tail, mat(0, 0.405, -1.008, 0, Math.PI, 0), Role.Base, 5);
...
export function buildKart(
  ...
  // --- driver -------------------------------------------------------------
  // Seated lower and a touch further back than the kart driving position —
  // legs-under-a-cowl instead of knees-to-the-wind.
  const driver: DriverRig = buildDriver(livery);
  driver.root.position.set(0, 0.355, -0.06);
  ...
  // --- effect anchors ------------------------------------------------------
  // Flames exit the twin low tailpipes, so the anchors follow them: low, at
  // the underside of the rear deck, blowing back and slightly down.
  const exDir = new THREE.Vector3(0, 0.22, -0.975);
  const exhausts = [
    anchor('exhaustL', -0.245, 0.295, -1.09, exDir),
    anchor('exhaustR', 0.245, 0.295, -1.09, exDir),
  ];
