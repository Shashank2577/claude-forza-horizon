...
import { Menus, currentMode, type ModeId } from './Menus';
...
export class HUD implements System {
  ...
  private onEvent = (e: import('../types').GameEvent) => {
    ...
    switch (e.type) {
      ...
      case 'finish':
        ...
        break;
      // PR stunts fire only in free roam. The stars ride IN the glyph — one
      // slot per star — because a separate "3 stars" line would need a third
      // text run on an element that exists for about two seconds.
      case 'stunt': {
        const tint = e.stars >= 3 ? '#ffd36b' : e.stars === 2 ? '#ff6ad5' : '#4fc3ff';
        this.toast(`${e.label}  ${'★'.repeat(e.stars) || '·'}`,
          e.kind === 'speed' ? '⚡' : e.kind === 'drift' ? '↯' : '⤴', tint);
        break;
      }
    }
  };
  ...
  lateUpdate(ctx: Ctx, dt: number) {
    ...
    // Mode-aware chrome. Roam has no laps, no clock and no one to race; sprint
    // keeps the clock and the position row but has no laps either. One root
    // class, so ui.css does the per-widget hiding and touch reflows inherit it
    // for free.
    const mode: ModeId = currentMode(ctx);
    this.root.classList.toggle('mode-roam', mode === 'roam');
    this.root.classList.toggle('mode-sprint', mode === 'sprint');
