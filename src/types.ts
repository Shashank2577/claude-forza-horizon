...
export const enum RaceState {
  ...
  Results = 4,
  Paused = 5,
  /** Open-world free roam: no laps, no countdown, no finish line. */
  FreeRoam = 6,
  /**
   * Sprint event: point-to-point over a sub-range of the circuit, one
   * "lap" that ends at the finish gate rather than back at the start line.
   */
  Sprint = 7,
}

/** How the current session is being played. FreeRoam/Sprint are Horizon-style modes. */
export const enum RaceMode {
  Circuit = 0,
  FreeRoam = 1,
  Sprint = 2,
}
...
export interface IRace extends System {
  ...
  /**
   * Which way this session is being played. The lap/finish machinery keys off
   * it: FreeRoam disables laps, wrong-way and the finish director entirely,
   * Sprint runs a single point-to-point "lap" between two stations.
   */
  readonly mode: RaceMode;
  /**
   * Enter free roam: karts scatter to scenic spots, no countdown, no laps.
   * The player just drives. Escape still pauses, and quitting returns to the
   * menu with `state = Menu`.
   */
  startRoam(): void;
  /**
   * Enter sprint: field lines up at station `fromT`, one flying run to
   * `toT`. Uses the normal countdown and finish plumbing but never wraps.
   */
  startSprint(fromT?: number, toT?: number): void;
  /**
...
export type GameEvent =
  ...
  | { type: 'stunt'; kind: 'speed' | 'drift' | 'danger'; stars: number; score: number; label: string }
  | { type: 'ui'; name: string };
